...
library(e1071)
#import data
testdata2<-read.csv("C:/Users/Admin/Documents/house_price/test.csv")
trainingdata2<-read.csv("C:/Users/Admin/Documents/house_price/train.csv")
...
combineddata$LandContour<- as.integer(combineddata$LandContour)
combineddata$LandSlope<- as.integer(combineddata$LandSlope)
combineddata$Neighborhood<- as.integer(combineddata$Neighborhood)
combineddata$BldgType<- as.integer(combineddata$BldgType)
combineddata$HouseStyle<- as.integer(combineddata$HouseStyle)
combineddata$RoofStyle<- as.integer(combineddata$RoofStyle)
combineddata$ExterQual<- as.integer(combineddata$ExterQual)
combineddata$ExterCond<- as.integer(combineddata$ExterCond)
combineddata$Heating<- as.integer(combineddata$Heating)
combineddata$HeatingQC<- as.integer(combineddata$HeatingQC)
combineddata$CentralAir<- as.integer(combineddata$CentralAir)
combineddata$PavedDrive<- as.integer(combineddata$PavedDrive)
combineddata$SaleCondition<- as.integer(combineddata$SaleCondition)
combineddata$Foundation<- as.integer(combineddata$Foundation)
...
#Data_Normalised <- as.data.frame(lapply(combineddata, FeatureScaling))
#combineddata <- Data_Normalised

trainingdata<-combineddata[1:1460,]

trainingdata <- matrix(trainingdata)

trainingdataTest <- normalize(trainingdata, axis =1)

testdata<-combineddata[1461:2919,]
#testdata$SalePrice <- NULL
str(trainingdata)


trainingdataY <- as.data.frame(trainingdata$SalePrice)
trainingdata$SalePrice <- NULL
